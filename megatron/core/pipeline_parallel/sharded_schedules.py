...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    ...
    if output_tensor_grad[0] is not None:
        print(f"rank={torch.distributed.get_rank()}, output_tensor shape={output_tensor[0].shape}, output_tensor_grad shape={output_tensor_grad[0].shape}", flush=True)
...
pre_guard_rank = 3 # hard coding for 8 stages
...
def quantize_3d(tensor, bits=8, signed=True):
    """
    Quantize a 3D tensor to the specified bit width.
    
    Args:
        tensor (torch.Tensor): Input tensor of shape (b, s, h).
        bits (int): Bit width for quantization (e.g., 8 for 8-bit).
    ...
    """
    # Determine the range for the target bit width
    if signed:
        max_val = 2 ** (bits - 1) - 1  # e.g., 127 for 8-bit signed
        min_val = -max_val - 1          # e.g., -128 for 8-bit signed
    else:
...
def dequantize_3d(quantized_tensor, scale):
    """
    Dequantize a 3D tensor using the given scale factor.
    
    Args:
        quantized_tensor (torch.Tensor): Quantized tensor of shape (b, s, h).
...
def scale_update(cur_scale_factor):
    global scale
    if torch.abs(cur_scale_factor - scale) / scale > 0.05:
        scale = cur_scale_factor
        return True
    return False
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    print(f"num_microbatches={num_microbatches}, num_warmup_microbatches={num_warmup_microbatches}", flush=True)
    # Checkpoint the activations of partial Transformer layers in a number of micro-batches
    # within the maximum outstanding micro-batch backpropagations.
    # Micro-batches with the ids less than 'num_microbatches_with_partial_activation_checkpoints'
    # checkpoint partial Transformer layers (or skip checkpointing) and
    ...
    print(f"rank={local_rank}, shapes for send/recv: {send_tensor_shapes}, {recv_tensor_shapes}", flush=True)
    ...
    print(f"rank {local_rank}, finish warm up phase.", flush=True)
    # Before running 1F1B, need to receive first forward tensor.
    # If all microbatches are run in warmup / cooldown phase, then no need to
    # receive this tensor here.
    if num_microbatches_remaining > 0:
        print(f"rank {local_rank} recv forward remained", flush=True)
        input_tensor = recv_forward(recv_tensor_shapes, config)
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            if last_iteration:
                input_tensor = None
                print(f"rank {local_rank}, send backward {i + num_warmup_microbatches}.", flush=True)
                send_backward(input_tensor_grad, recv_tensor_shapes, config)
            else:
                print(f"rank {local_rank}, send_backward_recv_forward {i + num_warmup_microbatches}.", flush=True)
                input_tensor = send_backward_recv_forward(
                    input_tensor_grad, recv_tensor_shapes, config
                )
    
    print(f"local rank={local_rank}, start cooldown phase.", flush=True)
