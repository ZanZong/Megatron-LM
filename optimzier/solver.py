...
from stropt.core.utils.solver_common import solve_r_opt
...
class ILPSolver:
    ...
    def solve(self):
        ...
        with Timer('Gurobi model optimization', extra_data={'T': str(T), 'budget': str(self.budget)}):
            ...
            print(f"Solve time of checkmate optimization with approximation={not self.integral}: {self.solve_time}s")
        ...
        def format_matrix(matrix, model: Model, label: str, shape: int):
            model.message(f"\n====== Solved Solution ====== {label}:")
            for i in range(shape):
                str_list = []
                if label == "Free_Eout":
                    for j in range(len(self.g.edge_list)):
                        str_list.append(str(matrix[i, j]))                    
                else:
                    for j in range(shape):
                        if label == "U":
                            str_list.append(str("%e" % matrix[i, j]))
                        else:
                            str_list.append(str(matrix[i, j]))
                model.message(" ".join(str_list))

        format_matrix(Rout, self.m, "R", T)
        format_matrix(Sout, self.m, "S", T)
        format_matrix(Uout, self.m, "U", T)
        format_matrix(Free_Eout, self.m, "Free_Eout", T)
