...
from typing import Tuple, Iterable, Dict, List, Set, Optional
import json
import numpy as np
from profiler.performance_model import CostModel
...
class DFGraph:
    def __init__(self, args: AdjList, v: Iterable[Vertex], vfwd_map: Dict[Vertex, Vertex], vloss: Vertex,
                 cost_time: Dict[Vertex, int] = None, cost_ram: Dict[Vertex, int] = None,
        ...
        :param cost_time: Dictionary mapping nodes to respective integral runtime costs (for forward/backward operators)
        ...
        :param symmetric: True is only required for maxmize the batch size and cannot be used when simplifing the graph
        """
        self.key = key
        ...
        self.cost_time = cost_time if cost_time else {v: 1 for v in set(self.v + [self.vloss])}
        self.cost_ram = cost_ram if cost_ram else {v: 1 for v in set(self.v + [self.vloss])}
        self.cost_time_np = np.array([cost for _v, cost in sorted(self.cost_time.items())])
        ...
        if symmetric:
            ...
            # Get profiling computation cost and memory cost from parameters for each node v
            self.cost_time_fwd = {v: cost_time[v] for v in self.vfwd} if cost_time else {v: 1 for v in self.vfwd}
    ...
    def cpu_gcd(self, *othervals):
        values = list(self.cost_time.values())
        values.extend(othervals)
        values = np.array(values)
        intvalues = np.array(values, dtype=int)
        if not np.allclose(intvalues, values):  # GCD is 1 if values are not integral
...
def gen_linear_graph(forward_node_count, **kwargs):
    ...
    Method returns forward and backward graphs. Pass cost_ram and cost_time as kwargs.
...
def dfgraph_transformer(input_dep=False, output_dep=True, next_outputs_deps=True,
    ...
    mems = {idx: 0 for idx in range(len(layers) * 2 + 1)}
    costs = {idx: 0 for idx in range(len(layers) * 2 + 1)}
    print(f"memory length={len(mems)}")
    
    for idx in range(len(layers)):
        mems[idx] = 0
        mems[fwd_to_bwd(idx)] = 0
        costs[idx] = 0
    ...
    # TODO Get parameter and gradient momentum memory usage
    # total_params = np.array(list(count_params_keras(mod)))
    # total_mem_params = total_params * MEMORY_MULTIPLIER
    total_mem_params = 0
    return DFGraph(args=args, v=vfwd + [loss_node_idx] + vback, vfwd_map=vfwd_map,
                           vloss=loss_node_idx, cost_time=costs, cost_ram=mems, node_names=names,
